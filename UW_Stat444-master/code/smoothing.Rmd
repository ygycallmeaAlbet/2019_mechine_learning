...
xg_at1 <- function(fold_num){
  ...
  #0.1956027
  #gam.object <- gam(PRICE~ s(BATHRM) + HF_BATHRM + s(ROOMS) + s(BEDRM)  + s(YR_RMDL)+ s(EYB) +  s(SALEYEAR) + s(SALEMONTH)+ s(GBA) +s(GRADE)+ CNDTN   + s(SALEDATE)+KITCHENS + s(AYB)+ AC+s(YR_RMDL) + s(LANDAREA) + te(LANDAREA, LONGITUDE,by =CNDTN)+ s(FIREPLACES) + s(ZIPCODE) + te(SALEDATE, ZIPCODE)+ te(SALEDATE, CENSUS_TRACT) + te(ZIPCODE,LATITUDE,LONGITUDE) + s(CENSUS_TRACT), data=train)
...
```{r}
rand_to_sub <-data.frame(Id=total$V1,PRICE=exp(total$predict_price))
write.csv(rand_to_sub,'../SmoothingToSubmit.csv',row.names = FALSE)
```
