...
Rand_Test <- function(data,fold_num){
  tic(paste('fold num',fold_num))
  ...
  predict_price<-predict(rf,test)
  RMLSE_Score(test_price,predict_price)
  toc()
}
...
gam.object <- gam(PRICE~.,data=train)
test<-data[data$fold==fold_num,]
...
bst <- xgb.train(data=dtrain, max.depth=9, eta=0.03, nthread = 4, nrounds=5000, watchlist=watchlist)
