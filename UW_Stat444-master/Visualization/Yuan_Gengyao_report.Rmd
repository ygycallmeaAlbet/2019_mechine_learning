...
library(mgcv)
```
The subset of [D.C. Residential Properties](https://www.kaggle.com/christophercorrea/dc-residential-properties) used in the project contain 37 variables, which are:               
Id\
...
```                    
Basically, by Observation results, the first 5 variables have the strongest relationship with PRICE,but as kitchen increase, PRCIE don't change a lot. Thus in the first part of the smoothing method later, we plug the first 5 variables into predict model first becuase they have strong relationship with PRICE.                     
...
data$EXTWALL[data$EXTWALL == 'Adobe'|
               ...
               data$EXTWALL == 'Plywood' ] <- 
                        ...
                        & data$EXTWALL != 'Default'& data$EXTWALL != 'Plywood' ]),size = 8)
...
```{r,echo=FALSE}
knitr::include_graphics("RandomForestMtry.jpg")
```               
...
```{r,echo=FALSE}
knitr::include_graphics("RandomForestDepth.jpg")
```     
...
```{r,echo=FALSE}
knitr::include_graphics("BoostingDepth.jpg")
```  
...
```{r,echo=FALSE}
knitr::include_graphics("BoostingnroundTesting.jpg")
```  

```{r,echo=FALSE}
knitr::include_graphics("BoostingnroundTraining.jpg")
```  
...
```{r,echo=FALSE}
knitr::include_graphics("BoostingVariableImportance.jpg")
```  
